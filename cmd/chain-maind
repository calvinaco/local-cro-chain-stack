...
Flags:
    ...
    --runtime:  The runtime to use under runtime/crypto-org-chain folder. Default: "validator0". Make sure the flag is
                passed before the command.
EOF
    exit 1
}

DOCKER_IMAGE=local-cro-chain/crypto-org-chain
HOST_PORT=$(docker inspect local-cro-chain-crypto-org-chain-validator0 | jq -r '.[0].NetworkSettings.Ports."26657/tcp"[0].HostPort')

RUNTIME=./runtime/crypto-org-chain/validator0
while [[ $# > 0 ]]; do
    case "$1" in
        --help)
            helpFunction
        ;;
        --runtime)
            RUNTIME=./runtime/crypto-org-chain/$2
            shift 2
        ;;
        *)
            break
